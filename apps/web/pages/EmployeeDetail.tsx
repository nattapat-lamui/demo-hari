...
import { MOCK_EMPLOYEES, MOCK_DOCUMENTS, MOCK_JOB_HISTORY, MOCK_EMPLOYEE_TRAINING, MOCK_PERFORMANCE_REVIEWS } from '../constants';
...
export const EmployeeDetail: React.FC = () => {
  ...
  // Other Data
  const [trainingRecords, setTrainingRecords] = useState(MOCK_EMPLOYEE_TRAINING.filter(t => t.employeeId === id));
  ...
  const [historyList, setHistoryList] = useState<JobHistoryItem[]>(MOCK_JOB_HISTORY);
  ...
  useEffect(() => {
    ...
    if (found) {
        ...
        setReviewsList(MOCK_PERFORMANCE_REVIEWS.filter(r => r.employeeId === id));
        setDocumentsList(MOCK_DOCUMENTS.filter(d => d.owner === found.name));
        setTrainingRecords(MOCK_EMPLOYEE_TRAINING.filter(t => t.employeeId === id));
    }
  }, [id]);
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                         <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${
                            ...
                            <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                                employee.status === 'Active' ? 'bg-green-500' : employee.status === 'Terminated' ? 'bg-red-500' : 'bg-yellow-500'
                            }`}></span>
                            {employee.status}
                        </span>
                    </div>
