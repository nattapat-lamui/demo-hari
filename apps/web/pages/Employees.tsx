...
export const Employees: React.FC = () => {
  ...
  const handleAddEmployee = (e: React.FormEvent) => {
    ...
    const newId = (Math.max(...employeesList.map(e => parseInt(e.id))) + 1).toString();
    const employeeToAdd: Employee = {
      ...
      avatar: `https://picsum.photos/id/${Math.floor(Math.random() * 100) + 1000}/200/200`,
      skills: []
    };
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                             emp.status === 'Active' ? 'bg-green-500' : emp.status === 'Terminated' ? 'bg-red-500' : 'bg-yellow-500'
                        }`}></span>
                        {emp.status}
                    </span>
                  </td>
